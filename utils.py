...
# def simple_feistel_round(left, right, key):
#     # Simple round function: pseudo random based on key and data
#     return (right, left ^ ((key + right) % 256))

# def pseudo_random_permutation(seed, index, N=20):
#     """
#     Stateless pseudo random permutation returning one unique integer from [0, N-1].
#     Assumes N <= 256 for simplicity.
#     """
#     # Split input index into left and right halves (4 bits each if N=16 for example)
#     left = (index >> 4) & 0xF
#     right = index & 0xF

#     # Round keys derived from seed
#     k1 = (seed * 31) % 256
#     k2 = (seed * 17) % 256

#     # 4 Feistel rounds
#     left, right = simple_feistel_round(left, right, k1)
#     left, right = simple_feistel_round(left, right, k2)
#     left, right = simple_feistel_round(left, right, k1)
#     left, right = simple_feistel_round(left, right, k2)
#     # Combine halves
#     permuted = (left << 4) | right

#     # Modulo N to keep in domain
#     return permuted % N
