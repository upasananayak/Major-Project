...
class SimpleCloudServer:
    ...
    def load_data_from_iot(self, filename="iot_to_cloud_dec.json"):
        ...
        try:
            ...
            self.physical_filename = self.stored_data.get("original_name", self.input_file)
            ...
            print(f"   Error loading data from IoT: {e}")
            return False
    ...
    def read_file_chunks(self, filename):
        ...
        actual_chunk_count = (len(data) + self.blocks_per_chunk - 1) // self.blocks_per_chunk
        expected_chunk_count = self.metadata.get("num_chunks")

        if actual_chunk_count != expected_chunk_count:
            raise ValueError(f"Verification Failed: File size mismatch. "
                            f"Expected {expected_chunk_count} chunks, "
                            f"but found {actual_chunk_count} chunks.")
    ...
    def generate_proof(self, stored_file="input.txt"):
        ...
        from utils import pedersen_commit, HHF
        q = self.ZK_PARAMS["q"]
        
        recalculated_tags = []
        for i, chunk in enumerate(self.file_chunks):
            m_i = int.from_bytes(bytes(chunk), 'big') % q
            r_i = self.random_values[i] % q
            
            commitment = pedersen_commit(m_i, r_i, self.ZK_PARAMS)
            tag = HHF(commitment)  
            recalculated_tags.append(tag)
        
        self.homomorphic_tags = recalculated_tags 
    ...
    def send_proof_to_verifier(self, filename="cloud_to_verifier.enc.json"):
        # print(f"\n Sending Hash-Based Proof to Verifier")
    ...
    def generate_proof(self, stored_file="input.txt"):
        """Generates the ZK proofs and Merkle paths and records the time."""
        ...
        for chunk_id, _ in self.challenge_set:
            aai_raw = self.generate_aai(chunk_id)  
            self.aai_data[str(chunk_id)] = [
                {"hash_value": h, "leaf_count": 2**i, "sigma": s} 
                for i, (h, s) in enumerate(aai_raw)
            ]

        self.challenged_tags = {
            str(cid): simple_hash(self.homomorphic_tags[cid]) 
            for cid, _ in self.challenge_set
        }
        ...
        self.gen_duration = time.perf_counter() - start_gen
        return self.proof

    def run_complete_workflow(self):
        try:
            self.decrypt_iot_payload()
            self.load_data_from_iot()
            self.receive_challenge()

            self.generate_proof(stored_file=self.physical_filename)

            start_veri = time.perf_counter() 
            
            from blockchain import Blockchain
            indices = [int(item[0]) for item in self.challenge_set]
            coefficients = [int(item[1]) for item in self.challenge_set]

            all_leaf_tags = [self.challenged_tags[str(i)] for i in indices]
            all_proof_hashes = [[node['hash_value'] for node in self.aai_data[str(i)]] for i in indices]
            all_leaf_counts = [[node['leaf_count'] for node in self.aai_data[str(i)]] for i in indices]
            all_sigmas = [[node['sigma'] for node in self.aai_data[str(i)]] for i in indices]

            bc = Blockchain()
            receipt = bc.verify_trustless_on_chain(
                self.input_file,
                all_leaf_tags,
                all_proof_hashes,
                all_leaf_counts,
                all_sigmas,
                self.proof["zk_proofs"]["m_sum"],
                self.proof["zk_proofs"]["r_sum"],
                indices,
                coefficients,
                self.z,                                  # nonce
                self.proof["zk_proofs"]["proof_value"]   # keccak hash
            )
            is_intact = bc.get_status(self.input_file) if receipt and receipt.status == 1 else False
            
            veri_duration = time.perf_counter() - start_veri 

            self.save_timings(self.gen_duration, veri_duration)

            if is_intact:
                print(" Blockchain confirmed: Data is INTACT.")
                return True
            return False
    ...
    def save_timings(self, gen_t, veri_t):
        filename = "timings_scripttwo.json"
        data = {}
        ...
        data["4. PROOF GENERATION (Cloud)"] = gen_t
        data["5. PROOF VERIFICATION"] = veri_t
