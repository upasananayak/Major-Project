...
contract StorageVerification {
    ...
    returns ( bytes32 rootHash)
   {
    ...
    function isFileDeleted(string memory fileId) public view returns (bool) {
        return files[fileId].isDeleted;
    }

    function getVersionHistory(string memory fileId) public view returns (uint256[] memory) {
        uint256 latest = files[fileId].latestVersion;
        uint256[] memory versions = new uint256[](latest);
        for (uint256 i = 1; i <= latest; i++) {
            versions[i-1] = i;
        }
        return versions;
    }
