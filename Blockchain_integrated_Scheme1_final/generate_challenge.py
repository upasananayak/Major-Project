...
def generate_challenge(c,num_chunks=None, iot_file="iot_to_verifier_dec.json"):
    ...
    if num_chunks is None:
        ...
        try:
            ...
            print(f"  Read num_chunks={num_chunks} from {iot_file}_challenge is {c}")
...
if __name__ == "__main__":
    # Check if an argument was passed from scripttwo.py
    if len(sys.argv) > 1:
        c_input = int(sys.argv[1])
    else:
        # Fallback to manual input if run individually
        c_input = int(input("Enter challenge size c: "))
    
    generate_challenge(c_input)
    print(f"Generated challenge with size: {c_input}")
