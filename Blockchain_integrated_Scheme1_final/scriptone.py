...
#     result = subprocess.run([sys.executable, "iot_simulation_dd.py"], text=True)
#     print("\n=== Store Completed ===")
...
def main():
    ...
    import sys
    # Read only one line to avoid waiting for EOF
    input_filename = sys.stdin.readline().strip() 
    if not input_filename:
        input_filename = "input.txt" # Default fallback
        start = time.perf_counter()
    #result=subprocess.run([sys.executable, "iot_simulation_dd.py"], check=True)

    # print("Input received by IOT:" ,input_filename)
    result = subprocess.run(
        [sys.executable, "iot_simulation_dd.py"],
