...
# Configuration - Small numbers as requested
BLOCKS_PER_CHUNK = 4    # n = 4 blocks per chunk (as requested)
BLOCK_SIZE_BITS = 8     # Each block is 8-bit integer (0-255, will be reduced mod 21)
...
class SimpleIIoTDevice:
    ...
    #     """Step 3: Generate homomorphic tags (Paper's key innovation)"""
    #     print(f"\n  STEP 3: Homomorphic Tag Generation")
        
    #     self.homomorphic_tags = []
    #     self.polynomial_commitments = []
        
    #     for i, chunk in enumerate(self.chunks):
    #         # Create polynomial from chunk: P_Fi(x) = chunk[0] + chunk[1]*x + chunk[2]*x² + chunk[3]*x³
    #         polynomial_coeffs = chunk
            
    #         print("polynomial_coeffs:", polynomial_coeffs)
    #         # Polynomial commitment: P_Fi(ψ)
    #         poly_commitment = eval_polynomial(polynomial_coeffs, self.secret_psi, P)
    #         self.polynomial_commitments.append(poly_commitment)
            
    #         # Homomorphic tag: ϖ_i ← HHF(P_Fi(ψ))  [This is the paper's formula]
    #         tag = HHF(poly_commitment)
    #         self.homomorphic_tags.append(tag)
            
    #         print(f"   Chunk {i}: P_F{i}(ψ) = {poly_commitment} → Tag = {tag}")
        
    #     print(f"   Generated {len(self.homomorphic_tags)} homomorphic tags")
    ...
    def generate_homomorphic_tags(self):
        ...
        for i, chunk in enumerate(self.chunks):
            ...
            # Evaluate polynomial at psi: P(ψ)
            poly_value = eval_polynomial(polynomial_coeffs, self.secret_psi, P)

            # Simple commitment with randomness (simulated commitment scheme)
            # Commit = (P(ψ) + r) mod P
            commitment = self.hash_commitment(poly_value, r)
    ...
    def upload_data(self):
        ...
        # Save to files
        with open('iot_to_cloud.json', 'w') as f:
            json.dump(cloud_data, f, indent=2)

        cloud_json = json.dumps(cloud_data).encode()
        # For testing: generate server KEM keypair here (in prod the server will have persistent keys)
        server_pk, server_sk = kem_keypair()
        ct, shared = kem_encapsulate(server_pk)

        encrypted_payload = encrypt_with_shared_key(shared, cloud_json)

        # Save the encapsulated ciphertext + encrypted payload
        with open("iot_to_cloud.enc", "wb") as f:
            f.write(ct + encrypted_payload)  # store ct || nonce || ciphertext
        # Also send server_pk if necessary (in tests)
        
        with open('iot_to_verifier.json', 'w') as f:
